...
Lecker is a custom-made Content Management System (CMS) based on CodeIgniter 3, written in PHP 8.2. It uses Composer for installing additional libraries.
...
- `application/`
  ...
  - `views/` - HTML views for frontend, mail, authentication, shop, etc.
...
3. Shop Controller
   - Checkout and payment processing functions

4. Authentication & User Controller
...
The Content Management System is accessible via the Backend at `/backend`. Key features include:
...
3. Image Repository
   ...
   - Saved in three versions using Imagick library:
     - Frontend image
     - Full size
     - Thumbnail
